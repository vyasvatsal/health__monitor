            ...
            <div class="bg-slate-900 border border-white/10 rounded-2xl flex flex-col">
                ...
                &lt;/script&gt;</pre>
                        ...
                        <div x-show="tab === 'laravel'" class="space-y-4">
                            <p class="text-sm text-slate-400">The AiHealth Laravel SDK automatically hooks into Laravel's exception handler. Just add your keys to your <code>.env</code> file.</p>

                            <div class="space-y-3">
                                <h4 class="text-xs font-semibold text-slate-300 uppercase tracking-wider">1. Install Package</h4>
                                <div class="bg-black/50 rounded-lg p-3 border border-white/5 relative group">
                                    <button class="absolute top-2 right-2 text-slate-500 hover:text-white"
                                        onclick="navigator.clipboard.writeText(this.parentElement.innerText.trim())"><i
                                            class="material-icons text-sm">content_copy</i></button>
                                    <pre class="text-xs text-slate-300 font-mono overflow-x-auto p-2">composer require aihealth/laravel-monitor</pre>
                                </div>

                                <h4 class="text-xs font-semibold text-slate-300 uppercase tracking-wider">2. Configure Environment</h4>
                                <div class="bg-black/50 rounded-lg p-3 border border-white/5 relative group">
                                    <button class="absolute top-2 right-2 text-slate-500 hover:text-white"
                                        onclick="navigator.clipboard.writeText(this.parentElement.innerText.trim())"><i
                                            class="material-icons text-sm">content_copy</i></button>
                                    <pre class="text-xs text-slate-300 font-mono overflow-x-auto p-2">AIHEALTH_DSN="{{ $store->secret_key }}"
    AIHEALTH_ENDPOINT="{{ url('/api/ingest') }}"</pre>
                                </div>
                            </div>
                        </div>
                    </div>
